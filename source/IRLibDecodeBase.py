...
class IRLibDecodeBase:
    ...
    def dumpResults(self, verbose=True):
        print("Decoded {} ({}): Value:{} Adrs: {} ({} bits)".format(
            IRLibProtocols.Pnames[self.protocolNum],
            repr(self.protocolNum),
            hex(self.value),
            hex(self.address),
            self.bits))
        if not verbose:
            return
        print("Raw samples({}):  Head: m{} s{}".format(
            len(IRrecvPCI.decodeBuffer),
            IRrecvPCI.decodeBuffer[0],
            IRrecvPCI.decodeBuffer[1]))
        for x in range(2, len(IRrecvPCI.decodeBuffer)):
            if x % 2:
                print("s{}".format(IRrecvPCI.decodeBuffer[x]), end=' ')
            else:
                print("{:2d}:m{}".format(
                    x // 2 - 1, IRrecvPCI.decodeBuffer[x]), end=' ')
            if (x % 2) == 1:
                print(end='\t')
            if (x % 4) == 1:
                print("\t", end=' ')
    ...
    def decodeGeneric(
        self, expectedLength, headMark, headSpace,
        markData, spaceOne, spaceZero
    ):
        self.resetDecoder()
        dataLow = 0
        dataHigh = 0
        ...
        if not self.ignoreHeader:
            if(headMark):
                if(not self.MATCH(IRrecvPCI.decodeBuffer[0], headMark)):
                    # HEADER_MARK_ERROR(headMark)
                    return False
        if(headSpace):
            if(not self.MATCH(IRrecvPCI.decodeBuffer[1], headSpace)):
                # HEADER_SPACE_ERpage andROR(headSpace)
                return False
        offset = 2
        while (offset < Max):
